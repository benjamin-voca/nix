...
let
  ...
  mkNamespace = name: { labels ? {}, annotations ? {} }:
    {
      apiVersion = "v1";
      kind = "Namespace";
      metadata = {
  ...
  mkServiceBase = name: namespace: type: ports: { selector ? {}, annotations ? {} }:
    {
      apiVersion = "v1";
      kind = "Service";
      metadata = {
  ...
  mkArgoHelmApp = {
    ...
    in baseSpec // lib.optionalAttrs (ignoreDifferences != []) {
  ...
  mkCNPGConnectionString = clusterRef: 
    "postgres://${clusterRef.username}@${clusterRef.host}:${toString clusterRef.port}/${clusterRef.database}";

  # ===========================================================================
  # Cloudflared / Tunnel Configuration
  # ===========================================================================

  mkCloudflaredConfig = { tunnel, credentialsFile ? "/etc/cloudflared/creds/credentials.json", metrics ? "0.0.0.0:2000", no-autoupdate ? true }: ingressList:
    {
      inherit tunnel;
...
CFGEof
        ...
        ''];
        volumeMounts = [{ name = "runner-data"; mountPath = "/data"; }];
        resources = {
      ...
      container-dind = {
        ...
        volumeMounts = [{ name = "runner-data"; mountPath = "/var/lib/docker"; }];
        resources = {
          requests = { cpu = "100m"; memory = "128Mi"; };
      ...
      spec = {
        ...
        template = {
          ...
          spec = {
            ...
            volumes = [
              { name = "config"; configMap = { name = "cloudflared-config"; items = [{ key = "config.yaml"; path = "config.yaml"; }]; }; }
              { name = "creds"; secret = { secretName = credentialsSecret; }; }
            ];
          };
        };
      };
...
" manifestStrings;
  ...
  presetWithCNPG = clusterRef: extraValues:
    let
      dbConfig = {
        database = {
          DB_TYPE = "postgres";
